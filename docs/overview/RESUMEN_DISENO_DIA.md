...
### `dia close-day`
Cierra la jornada y genera síntesis orientada a objetivo:

- lee eventos del día
- extrae objetivo de la bitácora (sección manual)
...
Genera automáticamente:
- **Resumen del día** (agregado a bitácora)
- **Análisis vs objetivo** (archivo separado)
- **Índice de resúmenes** (`daily_summaries.ndjson`)
...
- **Sección 4**: Incrementales automáticos (append-only)
- **Sección 5**: Resumen del día (generado por `close-day`)

Regla dura: `/dia` solo puede escribir a partir de la sección 3.

### Análisis vs objetivo (`analysis/YYYY-MM-DD_vs_objetivo.md`)

Generado por `dia close-day`:
- objetivo declarado
- plan esperado
- resultado real
- brechas detectadas
...
Ver especificación: [`docs/specs/SPEC_FORK_MIN_DIA.md`](../specs/SPEC_FORK_MIN_DIA.md).
...
- `dia close-day` (cierra jornada, genera resúmenes)
- generación de bitácora única por jornada
- resúmenes orientados a objetivo
- análisis comparativo vs objetivo

Principio rector: **Los eventos son materia prima. Los resúmenes son el producto.**